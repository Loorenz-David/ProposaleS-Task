...
Feature code lives under `src/features/<feature>/` and integrations under `src/lib/<system>/` per [03-feature-architecture.md](architectural_contracts/03-feature-architecture.md). Today that is one feature, [proposal-preparation](src/features/proposal-preparation/README.md), over four server-side integrations: [proposales](src/lib/proposales/README.md), [ai](src/lib/ai/README.md), `agent`, and `env`. Within the feature, `server/` holds the workflow and `server/actions.ts` the six Server Actions that expose it; the two transports in `client/` are the only browser modules that call them. `client/` also owns the workspace shell, the page-lifetime session runtime and tab strip, and the presentation layer, whose `client/view-models/` adapt the server's own schema types for the components. `client/fixtures/` is test data only, and `retirement.test.ts` keeps it off every shipping path.
...
- The browser-to-server seam: six thin Server Actions in `server/actions.ts`, a client transport composing each turn envelope and a second one reading line-item images, expected failures crossing as error DTOs rather than thrown errors, and the `COPILOT_LIVE_MUTATIONS` switch gating draft creation.
