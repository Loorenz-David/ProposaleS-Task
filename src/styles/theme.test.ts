...
describe("C4: test collection partitions the tree", () => {
  it("C4(a): every discovered *.test.ts(x) is claimed by exactly one project", () => {
    ...
    const libraryTestFiles = walk(
      ...
      (name) => name.endsWith(".test.ts") && !name.endsWith(".live.test.ts"),
    );
...
const DESIGN_01_RAMP_NAMES = [
  ...
  "color-paper",
  "color-paper-ink",
  "color-paper-ink-body",
  "color-paper-ink-meta",
  "color-paper-rule",
  "color-paper-rule-strong",
  "color-paper-hero-start",
  "color-paper-hero-end",
