...
import { fixtureFailedResult } from "../fixtures/failures.fixture";
import { deriveTabStatus, toTabViewModel } from "./session-tab";

function statusFor(overrides: Parameters<typeof fixtureSessionRuntimeRecord>[0]) {
  return toTabViewModel(fixtureSessionRuntimeRecord(overrides));
}
...
describe("tab status precedence", () => {
  ...
  it("C1(b): draft reference record is created", () => {
    expect(statusFor({ workflow: fixtureTerminalWorkflowState() })).toMatchObject({ status: "created", statusText: "Created" });
  });

  it("C1(c): clarification result record needs you", () => {
    expect(statusFor({ latestResult: { status: "clarification", questions: [] } })).toMatchObject({
      status: "questions",
  ...
  it("C1(d): proposition record is ready", () => {
    expect(statusFor({ workflow: fixtureWorkflowState() })).toMatchObject({ status: "ready", statusText: "Ready" });
  });
...
describe("tab status overlaps", () => {
  it("C2(a): in-flight beats draft reference", () => {
    expect(statusFor({ inFlightTurn: { turnId: "turn", kind: "brief" }, workflow: fixtureTerminalWorkflowState() }).status).toBe("working");
  });

  it("C2(b): in-flight beats clarification", () => {
    expect(statusFor({ inFlightTurn: { turnId: "turn", kind: "brief" }, latestResult: { status: "clarification", questions: [] } }).status).toBe("working");
  });

  it("C2(c): draft reference beats current proposition", () => {
    expect(statusFor({ workflow: fixtureTerminalWorkflowState() }).status).toBe("created");
  });

  it("C2(d): draft reference beats clarification", () => {
    expect(statusFor({ workflow: fixtureTerminalWorkflowState(), latestResult: { status: "clarification", questions: [] } }).status).toBe("created");
  });

  it("C2(e): clarification beats current proposition", () => {
    expect(statusFor({ latestResult: { status: "clarification", questions: [] }, workflow: fixtureWorkflowState() }).status).toBe("questions");
  });

  it("C2(f): failed result with proposition is ready", () => {
    expect(statusFor({ latestResult: fixtureFailedResult("tool_output_invalid"), workflow: fixtureWorkflowState() }).status).toBe("ready");
  });

  it("C2(g): failed result without proposition is open, not a seventh status", () => {
    expect(statusFor({ latestResult: fixtureFailedResult("tool_output_invalid"), hasStartedTurn: true }).status).toBe("idle");
  });
});

describe("tab view model", () => {
  it("C3(a): exposes status text and a status-specific dot treatment", () => {
    const viewModel = statusFor({ workflow: fixtureWorkflowState() });
  ...
  it("C6(b): derives the status through the exported pure function", () => {
    const record = fixtureSessionRuntimeRecord({ hasStartedTurn: true });
  ...
  it("adds unread text only for an inactive tab", () => {
    const record = fixtureSessionRuntimeRecord({ unread: 3 });
