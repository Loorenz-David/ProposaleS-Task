...
| Component / interaction | `components/*`, `hooks/*` | Render with props or a hook harness; assert on visible states for each flow-state variant; keyboard and focus behavior for interactive components. A feature store is tested through its own module, without rendering | Mocked Server Actions or `client/` adapters |
...
- **Components and hooks**: every variant of the flow-state union renders something intentional; error DTO messages are shown, not replaced; retry appears only when `retryable`; interactive elements are reachable and operable by keyboard, and focus lands where [05-client-architecture.md](05-client-architecture.md) §7 says it must. A feature store's transitions are asserted directly ([05-client-architecture.md](05-client-architecture.md) §5.1); its tests do not render components.
- **Ported UI**: behavior carried over from a prototype is proven by interaction tests at this layer, not by visual comparison. The port's classification of state ([16-design-prototype-porting.md](16-design-prototype-porting.md) §3) tells you what to assert: workflow state round-trips through the server, UI state does not. Styling is reviewed visually, not snapshotted.
