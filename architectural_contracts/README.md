...
| **Rendered animation interiors** | A composition rendered as a pure function of an authored time axis — today the product-demo animation on the intro's third step — is drawn in one fixed pixel coordinate space that its frame scales as a unit, and styles that space with the `style` prop rather than Tailwind utilities. It is a picture of the product, not production interface: its `px` are positions inside a picture and Tailwind's spacing scale does not describe them. The exemption is scoped to the interior. Every colour, font, shadow and keyframe still resolves to a `src/styles/theme.css` token — `theme.test.ts` C1(a) enforces that on these files like any other — and the chrome around the composition (frame, controls, layout) is ordinary Tailwind. Nothing outside a time-driven composition may cite this. Narrows [15-ui-styling-and-component-system.md](15-ui-styling-and-component-system.md) §3 only. |
