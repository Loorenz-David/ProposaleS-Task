...
|                               |                                                                                                                                                                                                                                                                                                     |
| ----------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Status**                    | `RATIFIED` (2026-09-05, by the owner, David, on the surface in §15.1 together with the four owner decisions recorded in §15; recorded in §16 round 2). Amended 2026-09-06 by owner decisions 5 and 6 (frontend dependency foundation, §4.1, §15), recorded in §16 round 3; status unchanged. Deepened 2026-09-06 by the round-1 mechanism inventory (§12A, ledger F8–F27), recorded in §16 round 4. Owner decisions 7–10 ratified that inventory's four recommendations in §16 round 5; status unchanged. Amended 2026-09-06 by owner decision 11 (persistent agent shell + session-controlled Main Application Surface; §1, §5.1, §5.3, §8.6, §15), recorded in §16 round 6; status unchanged. Deepened 2026-09-06 by the round-2 mechanism inventory (§12A.21–§12A.23, ledger F28–F30, in-place amendments to §12A.1, §12A.6, §12A.7, §12A.8 and §12A.17), recorded in §16 round 7. Owner decision 12 ratified its sole recommendation in §16 round 8. Amended 2026-09-06 by owner decision 14 (the V1 narrow-width floor, §12A.19, §15), raised by the phase-02 projection and recorded in §16 round 9. Three precision amendments folded back from the phase-02 review on 2026-09-06 (§12A.19 conditions 2 and 4, §12A.23's landmark-name clause), recorded in §16 round 10; status unchanged. Amended 2026-09-07 by owner decision 15 (a session can be closed without being activated first; §12A.5, §15), raised by the phase-03 projection and recorded in §16 round 11. Amended 2026-09-07 by owner decision 19 (the session status note is not built in V1; §12A.3, §12A.7, §15), together with two precision amendments to §12A.3's row 5 and §12A.4's event table, all raised by the phase-04 projection and recorded in §16 round 12; status unchanged and no owner decision is open.                                                                                                                                               |
...
| 5 | a turn has ever been started for this session | `idle` | "Open" |
| 6 | no turn has ever been started for this session | `empty` | "Empty" |

Rows 5 and 6 partition everything rows 1–4 do not match, so the function is total: row 6 is the exact negation of row 5, so no record can fall between them.

*Precision amendment, 2026-09-07 (phase-04 projection L3, §16 round 12).* Row 5 previously read "at least one turn has **completed** for this session", which is strictly narrower than row 6's negation: a record with a turn started, none completed and none in flight matched neither row, so the function was not total and the partition sentence above was false. The repair is forced rather than chosen — §12A.6 already states, in ratified text, that "§12A.3 row 6 assigns status `empty` when no turn has ever been started — **the negation of input (1) alone**", and §12A.6 input (1) is "a turn has ever been started for this session, including one currently in flight". Row 5 is that predicate's complement within what rows 1–4 do not match. Every record that was decidable before decides identically; only the previously undecidable record moves, and it moves to the value §12A.6 already implied. No owner decision was needed and none is raised.
...
| the latest result kind is `failed` **and** no proposition exists (a failed first run) | `idle` (row 5) — **not a two-row contention.** Only row 5 matches this record; the row is here because the case a planner must write is "`failed` is not a seventh status", and the pair above it is where the ordering actually bites. Labelled 2026-09-07 (phase-04 projection L4); the count stays seven and F10 is unchanged |
...
**Accessible representation.** The tab's accessible name carries, as text: the session title, the status text of the row above, and the unread count in words when non-zero (design 04 §5). **The status note is not built in V1** (owner decision 19, 2026-09-07): nothing in V1 can honestly narrate what a session is doing, so the note has no source, and an invented sentence would enter every tab's spoken name permanently. It returns with the surface that can produce it (master plan §11.3 follow-up 22). Status is never carried by dot colour alone, and `ready` and `created` are distinguished in the accessible name whether or not the specs later give them distinct dots (§5.3; design 04 open question 1 stays a design delta). The animated dot is hidden from assistive technology and does not animate under reduced motion.
...
| the tab is reordered or scrolled into view | no change |
...
*Precision amendment, 2026-09-07 (phase-04 projection R5, §16 round 12).* The reorder row previously read "reordered, **renamed**, or scrolled into view". No rename mechanism exists anywhere in the workspace and no phase introduces one, so that clause named an event with no subject and any row asserting it would have measured an empty set and reported green — the discipline §16 round 10 applied to §12A.19 condition 4. Rename is recorded as unmeasured in V1 rather than left implied; the event returns to this table with the mechanism that produces it.
...
**The close affordance reaches every tab** (owner decision 15, 2026-09-07). Closing is not conditional on activating first: each tab carries a close control revealed on hover and, equivalently, on keyboard focus, so the non-active close row above is reachable by both input paths. This is what gives that row's focus clause — "unless focus was inside the removed tab" — a producible subject; without it, a foundation whose tabs activate on focus can never place focus inside a non-active tab, and the clause is decoration.
...
|---|---|---|
| tab status, phase label | the session runtime record | §12A.3 |
...
2. no pane's content overflows its own pane horizontally, except inside a container that declares its own horizontal scroll (the tab strip is such a container by design). **"Declares" is operational and means an authored declaration or an explicit opt-in marker the container sets deliberately — never a computed value** (precision amendment, 2026-09-06, phase-02 review B3): CSS forces `overflow-x` to compute to `auto` whenever `overflow-y` is not `visible`, so a check that reads the computed value exempts every vertically scrolling pane from the horizontal rule, silently and at every width;
3. every interactive element named in §5 is reachable and operable by keyboard;
4. no text node is clipped to zero rendered width; text that does not fit ellipsizes or wraps, and its full value remains in the element's accessible name. **A condition with no subject is recorded as unmeasured, never asserted as green** (precision amendment, 2026-09-06, phase-02 review B5, the same discipline this section already applies to condition 1's permanent green): if no text in the surface under test actually elides at a width, this condition measures an empty set there and says so. **Planting an element that exists only to give the condition a subject is not permitted** — it makes the instrument's red a property of the fixture rather than of the product;
...
**The V1 floor is 780px (owner decision 14, 2026-09-06).** The named test set's floor is the same width as the specification's lower stated threshold, so **the set has three distinct widths**, not four: the designed wide width, 1100px, and 780px. Below 780px the product promises nothing: design 02 §3.3's "out of scope for V1, but must not corrupt" stays true as a statement of intent and is **not measured by this invariant**, because no width below the floor is in the set. A later initiative that wants the non-corruption half measured names a below-floor width and adds it here.

**Invariant (F26).** All five conditions hold at every width in the named test set, verified by rendering rather than by visual review. *Named mutation: give a content column a fixed width instead of a maximum; **condition 2** must redden at the narrowest width in the set.*

**Precision amendment (2026-09-06, planning fold-back).** The named mutation previously read "condition 1 **or** 2 must redden", which is a disjunction standing in for an expected outcome (charter rule 2). It is now condition 2, because condition 1 cannot fail under design 02 §2's root `overflow:hidden`: the document's own horizontal scroll is suppressed by construction, so a fixed-width column shows up as pane-level overflow, which is condition 2. Condition 1 is still asserted — it simply cannot be the mutation's witness. No product semantics changed.
...
**The accessible name of each landmark is owned by the shell** (precision amendment, 2026-09-06, phase-02 review S4). The invariant above is only *observable* once a second Main Application Surface state exists, which is phase 14 — so the rule is stated here, where the landmark is created and can be held to it: a landmark's accessible name comes from the shell, either a label on the landmark itself or a shell-owned heading whose text does not vary with the presented state, and **never from the content of the state it currently presents**. A name drawn from the presented state's own heading satisfies every check the creating phase can run, and breaks this section's invariant in the phase that first swaps the state.
...
### Decision 14 — The V1 narrow-width floor is 780 pixels (amendment, 2026-09-06)

*(Decision 13 is the theme-layer scope decision of 2026-09-06, recorded in the implementation master plan §6.5A because its subject is planning-level. Owner-decision numbering is project-wide, so this record is 14.)*

*(Decision 16 of 2026-09-07 — that the new-session control ships in phase 03 — is likewise planning-level and is recorded in the master plan §11.1 and in the phase-03 plan. Design 04 §2 and §3.5 already specify the control; the decision fixes only which phase builds it.)*

- **Decision.** The workspace promises to be usable and uncorrupted down to **780 pixels** of viewport width — the designer's own lower threshold — and no narrower. Narrower windows are explicitly unsupported for V1.
- **Rationale.** 780px is the narrowest width at which the agreed clamp still leaves a usable proposal column: below it the agent pane's 320px minimum wins by design (design 02 §3.2), and the main pane becomes a strip. Naming a smaller floor would promise usability the layout does not deliver; the owner's stated preference is that promising less than we deliver is cheaper to correct later than promising more.
- **Consequences.** §12A.19's named test set has **three** distinct widths (the designed wide width, 1100px, 780px), not four, and says so. `NARROW_WIDTH_TEST_SET` (master plan §6.4) carries three members. Phase 02's C4 measures the five conditions at those three widths. Design 02 §3.3's "below 780 must not corrupt" remains the design's intent and is **not** measured in V1; that limit is recorded rather than left implied.
- **Still deferred.** Any below-floor behaviour: the collapse-to-overlay suggestion in design 02 §3.3 (which §5.1 already declines to promote to product truth), and any measurement of non-corruption below 780px.

### Decision 15 — A session can be closed without being activated first (amendment, 2026-09-07)

- **Decision.** The user may close any session tab directly, including one they are not currently working in. The close affordance is exposed on **every** tab — revealed on hover and, equivalently, on keyboard focus — not only on the active tab.
- **Rationale.** Design 04 §3.2 shows the close control on the active tab only and raises the alternative as its own open question 2. Under that shape, closing a finished background session costs a detour: activate it (losing your place), close it, and be moved again by the close's own focus rule. The owner's answer: *"yes user should be able to close none active tabs ( tabs the user is not in at )."*
- **Consequences.** §12A.5's close table already states the non-active close case and its focus destination; that row now has an affordance that can produce it. **This resolves what was otherwise an unproducible fixture**: the mandated primitive activates a tab on focus, so under the previous shape focus could never rest inside a non-active tab, and the close table's first row — "unless focus was inside the removed tab" — had no way to be exercised at all. The hover affordance must have a keyboard-reachable equal, per F6 and standing rule 5; a control revealed only on hover is not reachable.
- **Design delta.** Design 04 §3.2's "Inactive: no close" and its open question 2 are superseded by this decision, recorded in master plan §11.2. No session edits the design specification.

### Decision 19 — The session status note is not built in V1 (amendment, 2026-09-07)

*(Decisions 20 and 21 of 2026-09-07 — moving the unread counter to phase 05, and shipping the reduced-motion pulse correction in phase 04 while holding its proof for phase 05 — are planning-level and are recorded in the implementation master plan §11.1, on the decision-13 precedent.)*

- **Decision.** The short descriptive note that design 03 §3.2 places above the conversation, and that design 04 §5 places in the tab tooltip, is **not implemented in V1**. The tab reads "‹title›, ‹status text›, ‹N› unread"; the agent surface's status line carries the status text and the phase label and no note.
- **Rationale.** The only honest source for a real note is the in-flight turn's own content, and nothing in V1 reads what the agent is doing (§12A.3's Forbidden list bars a status derived from thread content, a string test, or elapsed time; standing rule 2 bars the prototype's `tabState` narration). Every concrete example the specifications give — "Reading the transcript", "Pricing the three seatings" — was produced by the prototype's fake handlers. Building it now means an implementer inventing six sentences of product copy, which then enters every tab's **accessible name** and is read aloud on every tab switch. The owner accepted the projection's recommendation: *"recomendations are correct."*
- **Consequences.** §12A.3's accessible-representation paragraph drops the note; §12A.7's register row reads "tab status, phase label". Nothing a user can act on is lost: status, title and unread all remain, and F6's "no state is colour-only" is satisfied by the status text alone. Design 03 §3.2 and design 04 §5 are recorded as deltas in master plan §11.2, not edited (standing rule 7). The note returns with the surface that can honestly produce it, registered as master plan §11.3 follow-up 22.
- **Still deferred.** Any per-session narration, and any mechanism that would derive one from thread content.
...
**Round 9 (2026-09-06, owner resolution of the phase-02 projection's card 1).** Status stays `RATIFIED`. The phase-02 plan projection found that §12A.19's named test set presumes a V1 floor no artifact fixes, and that the phase cannot build one of its four required width checks without it. The owner chose the projection's recommendation.

- **Decision 14:** the V1 narrow-width floor is **780px**, the specification's own lower threshold. Recorded in §15; §12A.19 amended in place to state that the set therefore has **three** distinct widths and that below-floor non-corruption is intentionally unmeasured in V1.
- **Precision amendment to §12A.19's named mutation**, folded back from the same projection: "condition 1 or 2 must redden" becomes "condition 2 must redden", because condition 1 cannot fail under design 02 §2's root `overflow:hidden` and a disjunction cannot be an expected outcome (charter rule 2). No product semantics changed.
- **Nothing else moved.** No scope-ladder surface, ledger objective, state boundary, backend/domain contract or mechanism definition changed; F26's invariant is unchanged except for the mutation's precision.

**Round 10 (2026-09-06, precision fold-back from the phase-02 review).** Status stays `RATIFIED`; **no owner decision is open and none is raised**. The review found three places where a mechanism this document defines is not decidable as written, and each defect it found was an implementation reproducing the section's own ambiguity rather than departing from it. No product semantics, scope-ladder surface, ledger objective, state boundary, owner decision or backend/domain contract changed.

- **§12A.19 condition 2 — "declares its own horizontal scroll" now has an operational meaning.** The obvious implementation reads the computed `overflow-x`, and CSS forces that value to `auto` whenever `overflow-y` is not `visible`, so every vertically scrolling pane exempted itself from the horizontal-overflow rule. Phase 02 shipped exactly that, and a 4000px block planted inside the agent pane left all three condition-2 rows green. The condition now says the exception is an authored declaration or a deliberate opt-in marker, never a computed value. Any future phase adding a scrolling pane would otherwise have reproduced it.
- **§12A.19 condition 4 — a condition with no subject is recorded as unmeasured.** Nothing in phase 02's shell elides at any width in the named set, so the condition measured an empty set and reported green, and its named mutation proved only that two strings are compared. The section already applies this discipline to condition 1's permanent green; it is now general, together with the rule that planting a subject to satisfy a condition is not permitted.
- **§12A.23 — the accessible name of each landmark is owned by the shell**, stated in §12A.23 itself, next to the invariant it protects, so the phase that *creates* a landmark can be held to it. F30 already requires the name to survive every session operation, but that is observable only once a second surface state exists (phase 14). Phase 02 named its `main` from the idle state's own heading and pinned the coupling in an approved criterion — a defect no check available to phase 02 could have caught.

**Round 11 (2026-09-07, owner decision 15, raised by the phase-03 projection).** Status stays `RATIFIED`; **no owner decision is open**. One product decision, answered by the owner and folded upstream before any phase-03 prompt compiled.

- **§15 decision 15 and §12A.5 — a session can be closed without being activated first.** The projection found that §12A.5's close table opens with a non-active close case whose focus clause no fixture could produce: the mandated tab foundation activates a tab on focus, and design 04 §3.2 puts the close control on the active tab only, so nothing could put focus inside a tab that was both non-active and closable. The owner's answer makes the close affordance reach every tab, on hover and on keyboard focus alike. The close table itself is unchanged — this adds the affordance the table always presumed.

**Round 12 (2026-09-07, owner decision 19 and two precision amendments, raised by the phase-04 projection).** Status stays `RATIFIED`; **no owner decision is open**. One product decision, answered by the owner, plus two places where a mechanism this document defines was not decidable as written. No scope-ladder surface, ledger objective, state boundary, backend/domain contract or F-entry wording changed.

- **§15 decision 19 and §12A.3, §12A.7 — the status note is not built in V1.** The projection found that "status note" appears three times in this document and is defined nowhere, that its only concrete examples are prototype narration the same document forbids, and that two phase-04 criterion rows depended on it. Because the resolution is user-visible copy that would live permanently in every tab's spoken name, it was raised as an owner card rather than routed as a plan gap.
- **§12A.3 row 5 — the precedence chain was not total.** Row 5 read "at least one turn has **completed**" while row 6 reads "no turn has **ever been started**"; a record between them matched no row, so a six-member total union forced the implementer to choose an undeclared `else`, and both choices satisfied the two rows that were supposed to distinguish them. The repair is forced by §12A.6's already-ratified sentence that row 6 is "the negation of input (1) alone", so no previously decidable case changes.
- **§12A.4 — "renamed" named an event with no subject.** No rename mechanism exists in the workspace and no phase introduces one. Recorded as unmeasured in V1 rather than carried as a row that would measure an empty set and report green, applying §16 round 10's own discipline.
