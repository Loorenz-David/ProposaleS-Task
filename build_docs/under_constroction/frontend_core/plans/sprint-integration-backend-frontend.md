...
`src/app/page.tsx` (`maxDuration = 120`) · `src/features/proposal-preparation/README.md` (rewritten as a current-state document) · root `README.md` (Status, workflow preamble, Environment, Quality, Testing strategy, Deployment, Current scope, structure paragraph) · `master-plan.md` §3C.

**Stamp**, run in order on the closing tree:

| Command | Result |
|---|---|
| `npm run typecheck` | green |
| `npm run lint` | green |
| `npm test` | **905 passed**, 114 files |
| `npm run test:e2e` | **72 passed** (offline `chromium`) |
| `npm run build` | green; `maxDuration = 120` accepted, so the project is on Fluid compute |

`next-env.d.ts` and `tsconfig.tsbuildinfo` were regenerated by the build and restored.

**One observation about the offline E2E, recorded because someone will hit it.** Re-running `npm run test:e2e` *immediately after* `npm run build` produced 25 failures: the production build overwrites `.next`, so the `next dev` Playwright then starts has to recompile from scratch while the specs fire with 5-second expect timeouts. The failures were spread across specs that this sprint never touched (divider focus, tab geometry) and were all timeouts, not assertions about behaviour. With a warm server the same tree passes 72/72, twice. The stamp order and CI's order both run `test:e2e` **before** `build`, so neither hits it; only the accidental inverse does.

**Documentation impact review** (contract 14 §8.4). The triggers that fired: a new capability (the seam), a changed user flow, a new environment variable, changed test commands, a deployment change, and new limitations. Patched:

- **Feature README** — rewritten. It said "implemented, backend only, no transport" and "there is no Server Action, Route Handler or UI in this feature", both now false. It now describes the integrated flow from the browser, the two entry points and how they layer, the responsibilities split, the transport's intent-versus-envelope rule, the two failure channels, the `leaf.ts` reader and why it exists, the invariants, the exposure switch, and the real limitations (replacement appends, no progress reporting, reload loses the work, the five recipient rows).
- **Root README** — Status rewritten (it claimed the two halves "are not yet integrated"); the workflow preamble's "not yet wired together" removed; `COPILOT_LIVE_MUTATIONS` added to the environment table with its no-default rationale; Quality gained the live E2E command and its second real draft; Testing strategy gained the offline vertical slice and the two Playwright projects; Deployment gained `maxDuration`, Fluid compute and the posture choice; Current scope's fixture-era bullet replaced, its "Not yet built" section removed because the seam it named now exists, and the closing "future integration work" paragraph replaced with the system's actual limitations; the repository-structure paragraph rewritten.
- **Not patched, checked and still true:** the architecture contracts (no rule changed), `src/lib/proposales/README.md` and `src/lib/ai/README.md` (no adapter changed), `api-documentation/`. The phase 16/17 and Pass B plans are historical and were not edited.

Every internal link in both READMEs resolves (checked by script), and no "planned seam" or fixture-era wording remains in either.

### 15.9 Exit gate

| # | Row | Evidence | Held |
|---|---|---|---|
| 1 | The two fixture-era modules are gone; no production path imports a fixture | T-RET-1, T-RET-1b, T-RET-2 | yes |
| 2 | A browser brief reaches `prepareFromBrief` through `prepareTurnAction` | T-INT-1, T-INT-2, T-E2E-1 | yes (live half: T-E2E-2, owner) |
| 3 | Clarification round-trips | T-INT-1 | yes |
| 4 | A real proposition renders through unchanged components | T-INT-2; component diff is A1/A2/B1 only | yes |
| 5 | Human correction reaches `editProposition` and renders the server's value | T-INT-3, T-INT-3b | yes |
| 6 | Approval is explicit, one intent, submit-once | R1.7/R6.3 over real shapes; `main-application-surface.test.tsx` R6.1 | yes |
| 7 | Execution creates or recovers exactly one draft | T-INT-6, T-INT-7b | yes |
| 8 | Created renders the backend uuid and editor URL verbatim | T-INT-7 | yes |
| 9 | Applied Pricing from read-back only; unavailable carries no amount | T-INT-8, T-INT-8b, `created.test.ts` | yes |
| 10 | Session isolation holds | R1.1, R2.x unchanged | yes |
| 11 | Stale turns cannot mutate the wrong session | R1.4–R1.6 unchanged | yes |
| 12 | No privileged module in the client graph | T-RET-4, T-RET-4b, `isolation.test.ts`, `npm run build` | yes |
| 13 | No persistence introduced | T-RET-3 | yes |
| 14 | `npm test` green | 905 passed | yes |
| 15 | `npm run test:e2e` green offline; T-E2E-2 green for the owner | 72 passed offline | **offline yes; the live run is the owner's** |
| 16 | `npm run typecheck` green | — | yes |
| 17 | `npm run lint` green | — | yes |
| 18 | `npm run build` green | — | yes |
| 19 | Live suites collected only by the live config/project | `playwright test --list` finds no `*.live.spec.ts`; `LIVE_SMOKE=1` finds it; `vitest.live.config.mts` includes only `*.live.test.ts` | yes |
| 20 | Both READMEs describe verified behaviour; links resolve | §15.8 | yes |

### 15.10 Component changes, dependencies, and what is left to the owner

**Component files changed — two, both class A/B, none class C:**

| File | Change | Class |
|---|---|---|
| `components/workspace/main-application-surface.tsx` | A1: `onApprove` dispatches `{ kind: "approval" }`; the acknowledgment import is dropped. A2: `onCommitEdit` types the value for its leaf via `leafKindForPath` + `toLeafValue` | A — transport correction |
| `components/workspace/agent-surface.tsx` | B1: the composer dispatches a revision once a proposition exists, and offers no turn on a terminal session | B — presentation contract correction |

Every other component is byte-identical: `git diff --stat d02b799 -- components/` lists only these two production files.

**Dependencies added: none.** One was nearly required — `@testing-library/user-event`, for the vertical slice — and was not added; the slice drives the UI with `fireEvent`, as every existing component test does.

**Left to the owner:**

1. **Vercel environment.** Set all eight variables, `COPILOT_LIVE_MUTATIONS` included. Nothing was configured on Vercel by this session.
2. **Deployment posture** (§13 card 1). Undecided, so option B is documented as the default: public production with mutations `disabled`. Choosing A or C changes environment values only, never code.
3. **Fluid compute / `maxDuration`.** `npm run build` accepted `maxDuration = 120` locally, which indicates Fluid compute; confirm on the Vercel project, since the platform enforces the cap there.
4. **The live E2E run.** `LIVE_SMOKE=1 npm run test:e2e` — not run here, as instructed. It **creates one real draft** titled `[DISPOSABLE COPILOT E2E] <iso timestamp>` and prints its uuid; delete it afterwards and record the date, tree SHA and uuid here.
5. **The live Vitest suites.** `LIVE_SMOKE=1 npm run test:live` — unchanged by this sprint, last recorded green 2026-09-07.
6. **Client bundle size** (§9). The build output for Next 16 with Turbopack reports routes without byte sizes, so the zod-in-client cost was not measured. It is accepted for the MVP either way; measuring it is not a blocker and nothing was restructured.
