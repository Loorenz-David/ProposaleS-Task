...
| | |
|---|---|
| **State** | `MACHINERY_COMPLETE` — exit gate passed at commit `2353530`; see §9's final record and the closing commit `sprint pass B: machinery complete; fixture-era gate` |
...
## Exit gate — final record

All nine rows of §9 verified against the tree at commit `2353530` (WP10 checkpoint), immediately
before the final closeout commit below.

1. **R1.1–R8.8 green, M1–M15 applied/reverted/logged** — every row is evidenced in its own WP
   checkpoint entry above (WP1: R1.1–R1.9, M1–M2; WP2: R2.1–R2.5; WP3: R3.1–R3.15, M3–M4; WP4:
   R4.1–R4.9, M5; WP5: R5.1–R5.8, M6–M8; WP6: R6.1–R6.10, M9–M12; WP7: R7.1–R7.8, M13; WP8:
   R8.1–R8.8, M14–M15). Fifteen named mutations total, matching M1–M15 exactly.
2. **Pass A's tests unedited except the recorded amendment** — the one amendment is C3(i) in
   `use-workspace-session-store.test.ts`, changed from "the strip contains exactly one
   `closeSession(`" to "the strip contains zero and `use-close-guard.ts` contains exactly one",
   logged in the WP3 checkpoint with its reason (the close path moved behind the guard). No other
   Pass A test file was edited for any reason other than the WP1 record-shape update the checkpoint
   log already covers, and the WP9 fix-round's four test corrections (helper split, click-reliability
   helper, two copy corrections), which are this pass's own tests, not Pass A's.
3. **Approved guards green and untouched except the two marked ✎** — C3(i) (above) and the
   session-tab fixture-factory/overrides amendment (WP1). Verified again at this stamp: the C5(a–d)
   containment perimeter, the C1(a)/C1(b)/C7(b) theme scanners, the empty-`ui/` guard, and the Vitest
   partition sentinels are all still green (part of the 328/328 total below) and none was edited
   beyond the two ✎ rows.
4. **Full stamp, clean tree, SHA** — `npm run typecheck`, `npm run lint`, `npm test`
   (328/328), `npm run test:e2e` (80/80), `npm run build` all green in that order on a freshly
   started dev server (port 3000 confirmed unheld before the E2E run). The two files
   `next-env.d.ts` and `tsconfig.tsbuildinfo` were regenerated by `typecheck`/`build` and restored
   to their committed content per §0's rule; the tree was otherwise byte-identical to the commit
   before and after the stamp. Tree identity: commit `2353530` (`CHECKPOINT (not approved): sprint
   pass B WP10 fixture-era audit`), `git status --porcelain` empty both before and after.
5. **§7 and WP10 filled in** — §7's session-isolation table cites R-numbers now evidenced above;
   WP10's nine-point audit is recorded in its own checkpoint section with the command or inspection
   behind each row.
6. **Perimeter held** — `git diff --stat b6a477e..HEAD -- src/app/ architectural_contracts/
   ui_design/ src/lib/` is empty. `useDepartureGuard()` is mounted in `ProposalWorkspace`, not
   `layout.tsx`.
7. **File lists, amendments, dependencies, open items** — see below.
8. **Final commit** — `sprint pass B: machinery complete; fixture-era gate`, immediately following
   this entry.

### Files created (Pass B)

`hooks/use-close-guard.ts` (+ `.test.ts`), `hooks/use-departure-guard.ts` (+ `.test.ts`),
`hooks/use-status-announcement.ts` (+ `.test.ts`), `hooks/use-turn-dispatch.test.ts`,
`components/workspace/main-application-surface.test.tsx`.

### Files modified (Pass B)

`hooks/use-turn-dispatch.ts`, `hooks/use-workspace-session-store.ts` (+ `.test.ts`),
`hooks/use-inline-edit.ts` (+ `.test.ts`), `client/view-models/{clarification,failure,main-surface,
review,session-tab}.ts` and their tests, `components/clarification/clarification-panel.tsx`
(+ `.test.tsx`), `components/review/{inline-editable-value.test.tsx,proposal-review-surface.tsx}`,
`components/session-tabs/{session-tab-strip.tsx,session-tabs-constants.ts}`,
`components/workspace/{agent-surface.tsx,confirm-dialog.tsx,main-application-surface.tsx,
proposal-workspace.tsx}`, `e2e/proposal-flow.spec.ts`, `README.md`, this plan file.

### Prop / contract extensions beyond Plan A, with reasons

- `AgentSurface`, `SessionTabStrip`, `MainApplicationSurface` gained an optional `closeGuard` prop
  so one shared `useCloseGuard()` instance (owned by `ProposalWorkspace`) serves both the strip's
  close control and the review surface's discard control. Reason: the guard's dialog is a single
  workspace-level affordance; two independent instances would risk two dialogs disagreeing about
  the same session.
- `ProposalReviewSurface` gained an optional `isSubmitting` prop disabling the approval control and
  edit affordances while *any* turn for the session is in flight, not only an edit turn. Reason:
  §12A.14/§12A.15 require the approval control and edit affordances to share the session-level
  in-flight rule, and the existing `isEditSubmitting` alone under-covered a pending brief, revision,
  or approval turn.
- `main-application-surface.tsx`'s `ProposalReviewSurface` element gained `key={activeSessionId}`.
  Not a prop on the component's public contract — an internal reconciliation key, added during the
  WP9 investigation as a defensive correctness fix (see that checkpoint entry) and not itself the
  fix for the reported symptom.

### Dependencies added

None. `package.json` and `package-lock.json` are unchanged from the Pass A baseline.

### Open items handed to the integration sprint (phases 16–17)

- Replace `client/fixtures/turns.temporary-fixture.ts` and its `TemporaryTurnInput`/
  `TemporaryTurnOutcome` shapes with the real Server Action boundary and the merged backend
  schemas; `TEMPORARY_FIXTURE_TURN_LATENCY_MS` is deleted with the fixture, not carried forward.
- Every other `*.temporary-fixture.ts` module becomes the parse result of its owning schema,
  flowing through a production adapter, per master plan §6.6 — no view model or component changes
  expected (F15's claim).
- The `WorkspaceSessionId`-cast at `session-tab-strip.tsx:161` and the guarded `ErrorTreatmentKey`
  cast in `failure.ts` are reviewed above and are expected to survive the seam unchanged.
- `use-close-guard.ts`'s meaningful-work predicate reads `record.workflow?.draftReference` etc. from
  the temporary workflow shape; once `workflow` becomes the real `ProposalWorkflowState`, the same
  field-presence checks apply unchanged (the predicate was written against the *shape* the intention
  describes, not the temporary type's incidental structure).
- No backend-owned contract, constant, or enum was invented in Pass B; every closed-set value
  (`ErrorTreatmentKey`, the five domain result statuses, the edit-operation kinds) is either
  imported from `src/lib/errors/error-dto.ts` or matches the backend intention's own naming exactly,
  so the merge should rename nothing on this side.
