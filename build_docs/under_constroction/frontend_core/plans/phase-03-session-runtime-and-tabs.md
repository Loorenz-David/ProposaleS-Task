...
| **C7** | Creating a session, and the control that does it. **Added 2026-09-07 by owner decision 16**, which puts the new-session control in this phase; the creation semantics it exercises were previously asserted by no row in this plan. **Runner: Vitest `node` for (a)–(c), `jsdom` for (d)–(e).** (a) Creating a session appends it at the **end** of the ordered list, leaving every existing id in its relative order (§12A.5, "Order"). *Named mutation: insert the new session at index 0; (a) must redden.* (b) The created session becomes the active session. (c) Its runtime record is a separate, empty record — no field is shared with, copied from, or serialised out of any other session's record (§12A.1, "Records are separate per session"). (d) The control carries an accessible name and is reachable by keyboard. (f) **The first rendered document contains at least one tab** (added 2026-09-07, review round 3 S11): the server-rendered markup carries no `role="tablist"` that owns no `tab`, and the new-session control is not inert on first paint. *Named mutation: move the initial session's creation back into a post-mount effect; (f) must redden.* (e) The control is a **sibling of the tablist, never a child of it**: a non-tab child inside a `tablist` is an accessibility defect and would join the roving-focus group, making it an arrow-key target (projection L24). Design 04 §2 places it outside the scroll region, pinned. | 6 | §12A.5 · F12 · F6 |

**Derived totals for this phase** (re-derived at source after routing the projection ledger,
2026-09-07; re-derive again at dispatch): **7 criteria, 47 rows** — C1 5 · C2 8 · C3 9 · C4 6 ·
C5 7 · C6 6 · C7 6 — of which **43 are measurable in this phase** and 4 are structurally held
with the triggers their cells name (C1(c), C1(d), C1(e), C6(d)). **18 runnable named mutations** —
C1(b) 1 · C2(b) 1 · C2(c) 3 · C3(a) 1 · C3(e) 1 · C3(g) 1 · C3(h) 1 · C3(i) 1 · C4(f) 3 ·
C5(c) 1 · C6(e) 1 · C6(f) 1 · C7(a) 1 · C7(f) 1 — plus **1 held** (C1(e)), which converts with the rows it serves and is not
...
### Review round 3 routed — coordinator, 2026-09-07, under owner decision 17

**Owner decision 17** — *"we are to focus on keyboard behaviour, this is not a must for the first
mvp, i prefer progress over keyboard improvements ( users don't use short cuts that much specially
for this app )"* — resolves card 1 by its own stated default (every close control keeps its tab
stop) and sets the routing rule for the rest: **keyboard refinement is deferred; user-visible
defects and unstable evidence are not.**

**Sorted on that rule, and the sort is the point.** Four of the eighteen findings survive into a fix
round, and **three of the four are things a user meets on the first screen** — none of them is
keyboard work, and one of them is a pointer interaction that the word "focus" in its write-up makes
look like keyboard work when it is not.

| Fix now | Why it is not keyboard |
|---|---|
| **B3** — closing a background session switches the active session | The trigger is a **mouse click on the ✕**. The activation happens because the close's own focus repair fires `onFocus`, but the user's action is a click; no shortcut is involved. It violates §12A.5's close table row 1 and ledger F12 |
| **S10** — the strip renders at the **bottom** of the Agent Surface | Design 03 §2 places Session Tabs directly under the header. This is layout, visible on load, and an undeclared deviation with no §11.2 row (standing rule 7) |
| **S11** — the server-rendered document carries an empty `role="tablist"` and an inert "New session" button | First paint. A `tablist` owning no `tab` is an ARIA structural violation, and the strip's only control does nothing until hydration |
| **C4(b) unstable** (coordinator addition) | Phase 03's own browser evidence for its own guarantee fails 3 of 4 isolated runs on the handed-over tree. An approval stamp taken once on an unstable suite is not evidence |

**Deferred to master plan §11.3 follow-up 19**, each a test-instrument repair with **no current
user-visible defect behind it**: B1, B2, B4, B6, S5, S6, S7, S8, S9. **B2 carries the one downstream
consequence** and is not simply parked — phase 05 inserts the unsaved-work guard at C3(i)'s gate, so
phase 05's pre-dispatch lint verifies the single close path by enumeration before dispatching. That
check costs a coordinator command and lands exactly where the risk does.

**Deferred with the keyboard decision:** S3 (the shell's tab-stop count growing with session count),
S5's enumeration gap, and S4's keyboard half. **Re-assigned:** B5 → master plan §11.3 follow-up 20,
phase 04, whose perimeter already includes source under the scanned trees. **Recorded, not
actioned:** S1 and S2 are process findings against the round and stand as written.

**What this costs, stated plainly so the trade is visible.** Phase 03 will be approved with nine
guards that cannot observe what they forbid. Every one of them is currently *passing over correct
code* — the review attacked the behaviour directly and found it sound on every path but B3 — so what
is deferred is protection against future regression, not a present defect. The phase's behavioural
