...
# Session prompt — implement phase 03 (round 2)
...
## 0. What happened to round 1, and what it means for you

**Round 1 stopped before writing any production code, and it was right to.** Its prompt grounded
the tab foundation's keyboard behaviour against inspected package versions and required a
stop-and-report on the first contradiction; the round installed the dependency, found one, reverted
`package.json` and `package-lock.json`, and reported. **That was correct behaviour and is recorded
as such** in the plan's Review log under "Round 1 stopped". You are not repairing an implementer's
defect and you are not resuming a partial implementation: the tree carries no phase-03 production
code, no dependency, and no handoff.

The contradiction was in the dispatch artifacts, and the diagnosis in the stop report was itself
incomplete — **do not go looking for a version to pin.** `TabsList` destructures `loop = true` in
every release involved, including the one originally inspected; the coordinator had read the inner
roving-focus primitive's `loop = false` and attributed it to the composite. The owner's decision and
the corrected requirement are in §3A. Everything else the round observed stands, including its
baseline, which §8 addresses.
...
outstanding, and the plan's counts are the amended ones — **7 criteria, 46 rows, 16 runnable
...
| 7 | The round is genuinely outstanding | `handoffs/implementer/` | no `phase-03-round-2` handoff exists |
| 8 | **The known intermittent end-to-end row is green** | the tree | `npx playwright test e2e/workspace.spec.ts -g "C2\(a\): :focus-visible"` passes. See §4A — if it is red, this is a **known** intermittent, not a discovery; follow the charter's flaky-capture rule there rather than treating it as a new finding |
...
Grounded by the coordinator on 2026-09-07 against the versions npm actually resolves —
**`@radix-ui/react-tabs@1.1.21` and `@radix-ui/react-roving-focus@1.1.19`** — read at source from
the package tarballs. **Install what npm resolves for `@radix-ui/react-tabs`; pin nothing.** Record
the resolved versions in your Review log. The first contradiction between any fact below and the
version you install is still a stop-and-report, not a silent adaptation — that condition is what
made round 1 useful.
...
- Roving focus: `tabIndex: isCurrentTabStop ? 0 : -1`; the key map is `ArrowLeft`/`ArrowUp` →
  previous, `ArrowRight`/`ArrowDown` → next, `Home`/`PageUp` → first, `End`/`PageDown` → last, and
  at **horizontal** orientation `ArrowUp`/`ArrowDown` are filtered out and move focus nowhere.
  C5(c) asserts exactly this set.
- **Wrapping is yours to decide, and the decision is already made: it does not wrap.**
  `RovingFocusGroup.Root` defaults `loop` to `false`, but the application never mounts that
  directly — `TabsList` sits in between and destructures **`loop = true`**, overriding it. This is
  true in `1.1.21` and was equally true in `1.1.13`; it is not a version change. `loop` is public,
  typed API on the composite
  (`interface TabsListProps extends PrimitiveDivProps { loop?: RovingFocusGroupProps["loop"] }`),
  so **set it explicitly to `false` on the tablist**. **Owner decision, 2026-09-07:** non-wrapping
  session-tab navigation is application behaviour, not a dependency-default contract; keep the
  resolving release and own the behaviour. Mutation 16 reddens if the explicit setting is removed —
  a row that passes only because a default agrees with us cannot fail when the default moves, which
  is exactly how this row was wrong at dispatch (standing rule 19).
...
## 4A. One known intermittent in the inherited suite — read before you report it

`e2e/workspace.spec.ts:92`, `C2(a): :focus-visible produces a visible indicator on an injected
native control`, fails intermittently at `:107`. **Cause, instrumented directly:** the suite runs
against `npm run dev`, and the Next.js dev-tools overlay renders a `<nextjs-portal>` element that is
a **tab stop**. That row counts three absolute `Tab` presses to reach a probe appended at the end of
`<body>`; when the overlay has mounted in time it takes the third press and the probe never receives
focus. The overlay exists **only** under `next dev` — `next build` / `next start` emit nothing of the
kind — so no shipped tab order contains it and no product invariant is involved. Measured on the
phase-02 tree: 6 failures, then 9 consecutive passes across a dev-server restart, with no repository
change between them. Registered as master plan §11.3 follow-up 18.

**What to do if you see it.** Apply the charter's flaky-capture rule: capture the failing-ID set,
re-run, and record both results. Do **not** weaken, re-baseline or delete the assertion to recover a
green run, and do not count it as one of your five re-baselined instances — it is not one of them.
If it fails **persistently** on your tree, stop and report: that would be a different finding from
this one.

**Why it matters to you specifically.** This phase inserts a tab stop into the same shell and
re-baselines the four other rows that count absolute `Tab` presses. When you compute those new
expected orders, compute them from the **product's** tab stops — skip link, divider, tablist, and
whatever this phase adds — and never from an observed count that may include the overlay. If a
re-baselined order only holds when the overlay is present, you have baked the dev server into
phase-02 evidence, which is a blocking defect rather than a re-baseline.
...
## 6. The sixteen named mutations — enumerated, all sixteen must run
...
**`executed != declared` blocks `IMPLEMENTED`.** The plan declares sixteen runnable; the summands
are C1(b) 1 · C2(b) 1 · C2(c) 3 · C3(e) 1 · C3(g) 1 · C3(h) 1 · C3(i) 1 · C4(f) 3 · C5(c) 1 ·
C6(e) 1 · C6(f) 1 · C7(a) 1 = **16**. Re-derive this from the criteria yourself and state your arithmetic.
...
| 13 | C5(c) | remove the explicit non-wrapping configuration at the tablist boundary, leaving the dependency default in force | C5(c)'s two no-op rows — `ArrowRight` on the last tab and `ArrowLeft` on the first |
| 14 | C6(e) | push a history entry on session activation | C6(c) |
| 15 | C6(f) | remount the Agent Surface when the active session changes | C6(b) |
| 16 | C7(a) | insert the new session at index 0 instead of the end | C7(a) |
...
1. **The baseline re-enumeration** — `npm test`, `npm run test:e2e`, `npm run typecheck`,
   ...
   `3796dc1`. **Round 1 measured 65/66**; that one row is the known intermittent of §4A, and the
   suite was green at 66/66 when this prompt was compiled. If your baseline differs from **154 unit
   and 66 end-to-end green** in any *other* respect, that is a finding before you have written a
   line.
...
4. **The closing L4+ stamp** (§8), with tree identity and the failure-ID delta against your
   baseline.
5. **All sixteen mutations run and reverted** (§6), each a full evidence record naming the site
...
8. **Review log entry** in the plan file: what you built, every delegated decision with its
   ...
   needs. **Four items are required by name**: how you carried the tabs relationship without
   making the landmark the panel (task 3); the resolved version of every package you installed and
   any contradiction with §3A's grounded facts; where you set the explicit non-wrapping
   configuration and why that boundary; and how the strip satisfies the two frozen phase-02 rows
   in §4.
...
10. **Handoff** at `handoffs/implementer/phase-03-round-2.handoff.implementer.md`, frontmatter
    `plan`, `role: implement`, `round: 2`, `state`, `date`, `actor`. Body: the coverage map, the
