...
| 04 | Derived presentation: status and the derivation register | `plans/phase-04-derived-presentation.md` | 5 | `PROMPT_READY` | 2026-09-07 | coordinator | projection round 0 `AMENDMENTS_REQUIRED`, all 24 rows routed, 3 owner cards answered (decisions 19, 20, 21). Unread and attention moved to phase 05 with decision 20; `C4`/`C5` retired, not reused. Now **5 criteria, 24 rows (2 held), 5 named mutations**. Four coordinator additions on consumption, including a perimeter-vs-guard collision the projection did not reach — a "derivation register" named `DerivationRegistry` reddens a phase-02 scan of all of `src/features` |
