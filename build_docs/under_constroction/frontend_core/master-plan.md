...
| 04 | Derived presentation: status, unread, the derivation register | `plans/phase-04-derived-presentation.md` | 6 | `NOT_STARTED` | 2026-09-07 | coordinator | projection required (§7.2, derivations). Pre-dispatch lint run 2026-09-07 under owner decision 18: the `AgentStatusLine.tsx` path corrected to kebab-case, the mutation count corrected (stated 4, listed 5), rows **39 → 34**. The proposed ≤25 cap was **refused with its reason recorded** — 22 of the rows are three total case tables over derivations, and sampling them is what rule 2 forbids and how a precedence chain ships silently wrong |
