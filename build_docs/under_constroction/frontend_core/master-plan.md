...
| 03 | Session runtime and the tab strip | `plans/phase-03-session-runtime-and-tabs.md` | 7 | `CHANGES_REQUESTED` | 2026-09-07 | reviewer | Review round 3 (first review). **Six blocking, eleven should-fix, nine notes.** The two coordinator findings confirmed and adjudicated (B1's correct instrument named: make `loop={false}` load-bearing by letting the roving-focus group own horizontal movement, or drop it and mutate the clamp). Four new blocking: **B3** closing a focused non-active tab silently switches the active session, violating §12A.5's close table row 1 and F12 — the phase's one behavioural defect, proved by interaction, and C3(a)'s test asserts only the focus half; **B4** C1(b) cannot detect a module-level counter placed inside the generator — the construct §12A.1 forbids and projection F1 re-authored the row to catch — 52/52 green under it; **B5** task 8's inherited repair (§11.3 follow-up 16) was neither implemented nor declared and `workspace.test.tsx` is untouched; **B6** C4(c)/(d)/(e) are bypassed by computed member access and `globalThis`, and C4(f)'s probes planted denylist-shaped defects, so standing rule 17's probe clause is unmet. Behaviour is otherwise sound on every path attacked, no orphan tests, and the frozen phase-02 perimeter held. One owner card: should every session's close control be its own tab stop (the strip contributes N+2 stops today). L4 budget zero — the stamp was cited, eleven L1 records taken by variation. `handoffs/reviewer/phase-03-review-round-3.handoff.reviewer.md` awaits the coordinator Routed by the coordinator 2026-09-07 under **owner decision 17**: four items to a fix round — B3 (a pointer click on a background tab's close control changes the active session), S10 (the strip renders at the bottom of the Agent Surface), S11 (the first rendered document carries an empty tablist and an inert control) and the coordinator's unstable `C4(b)` — none of them keyboard work. Everything else deferred to §11.3 follow-ups 19 and 20. Plan amended: C3(a) now asserts both clauses, **C7(f)** added, rows 46 → 47, runnable mutations 16 → 18. `prompts/implementer/phase-03-fix-round-4.prompt.implementer.md` is live and awaiting the owner's session |
...
| 20 | **§11.3 follow-up 16's repair was assigned to phase 03 task 8 and neither implemented nor declared** (review round 3, B5). `workspace.test.tsx` is untouched; the navigation denylist still misses `location.href`. Charter rule 14 requires an unimplemented quoted correction to be declared with its reason. Re-assigned rather than forced into phase 03's fix round | this project | phase 04, whose perimeter already includes source under the scanned trees |
