...
| 03 | Session runtime and the tab strip | `plans/phase-03-session-runtime-and-tabs.md` | 7 | `IMPLEMENTED` | 2026-09-07 | coordinator | Fix round 4 consumed: perimeter exact (nine files), coverage 47/4 held, mutations 18 = 16 + 2, unit 184/184, typecheck/lint/build green. **All four corrections landed and were independently verified** — B3's focus-repair guard traced through every caller to confirm it suppresses no activation that is required, S10's strip position, S11's module-scope seed with all three readiness waits retired, and C4(b) now **4/4** where it was 3/4 failing. **Approval blocked by §11.3 follow-up 18, not by this round**: `e2e/workspace.spec.ts` `C2(a)` is now deterministically red because the `next dev` dev-tools overlay occupies tab stop 6 and the probe is stop 7. Product tab order and the test's count are both correct. Awaiting the owner's choice on the evidence repair |
