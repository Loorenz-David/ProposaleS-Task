...
| **Phases** | 15: phases 1–3 `APPROVED`, phase 4 `REVIEWING`, phases 5–15 `NOT_STARTED` (§4) |
...
| 4 | Proposales adapter: create, recovery search, read-back, Applied Pricing | `plans/phase-04-proposales-proposals.md` | `IMPLEMENTING` | 2026-09-05 | coordinator | Fix round 2 dispatched from independent review: 3 blocking and 8 should-fix corrections; amended plan declares 8 criteria / 80 rows / 35 mutations. |
...
Criteria total: 103; rows: 542; named mutations: 123 — derived from the phase acceptance tables on 2026-09-05 after the phase-4 review fold: 5 + 7 + 6 + 8 + 8 + 8 + 7 + 6 + 6 + 6 + 8 + 8 + 7 + 8 + 5 criteria; 22 + 52 + 44 + 80 + 61 + 45 + 28 + 26 + 22 + 25 + 28 + 33 + 26 + 32 + 18 rows; 11 + 19 + 9 + 35 + 5 + 5 + 3 + 4 + 4 + 5 + 7 + 4 + 4 + 4 + 4 mutations. A criterion is a distinct `C<n>` in a phase table; each table line is one row unless its ID explicitly spans letters. Re-derive after any plan amendment; never edit these numbers by hand.
...
| `MAX_TITLE_CHARS`, `MAX_NARRATIVE_CHARS`, `MAX_COMMENT_CHARS`, `MAX_NOTE_TEXT_CHARS`, `MAX_QUESTION_CHARS`, `MAX_ANSWER_CHARS`, `MAX_RATIONALE_CHARS`, `MAX_WARNING_CHARS`, `MAX_ASSUMPTION_CHARS`, `MAX_QUOTE_CHARS`, `MAX_INSTRUCTION_CHARS` | `schemas/shared.ts` | 200 / 6000 / 500 / 500 / 300 / 2000 / 1000 / 500 / 300 / 300 / 2000 | each positive int; the field trims and rejects above cap (§17A.16) |
...
| `projectProvenance` | `domain/provenance-projection.ts` | `(p: Proposition) → Array<{ path, source, ref? }>` sorted by path |
...
| Propositions | `features/proposal-preparation/fixtures/propositions.ts` | `propositionWithAlternatives()` (block `A` with alternatives `[B, C]`, block `D`; ids from `FIXTURE_CATALOG`), `maximalConformingProposition()` |
...
| 22 | 4.C1, 5.C2, 14.C3 |  |
| 23 | 5.C5, 11.C5, 11.C7, 15.C3 | 5.C5(d) currency representable · 11.C5(c) language structural half · 11.C7 currency warning (card 1 → A) · 15.C3 eval half |
