...
**Resolved at the provider boundary, not by reshaping the domain.** `src/lib/ai/client.ts` now sends `providerOptions.openai.strictJsonSchema: false` on any step that carries an output schema, and `src/lib/ai/openai-schema.ts` adapts the schema itself for that provider only: it drops every `propertyNames` (lossless — JSON object keys are strings by definition) and declares `type: "object"` at the root when every branch of the union is an object. A union with a non-object branch is deliberately left untyped, to fail loudly rather than be mislabelled. Reshaping the schemas to satisfy strict mode would mean rewriting `refSchema`, `sourcedOrAbsent` and `warningSchema` — shapes the naming registry, §6.4, the proposition schema and the whole test suite are built on — to make the *internal* contract worse (a `ref` with five always-present nulls) for one vendor's decode mode. Turning strict off costs the vendor's conformance guarantee and nothing else: model output becomes authoritative only after `run()` parses it with the Zod output schema and `validateAgentOutput` resolves every reference, and nonconforming output is already a retried, then failed, turn. The option is namespaced by provider, so it is inert for Anthropic. Rows **C6(l)** and **C6(m)** pin the provider option; **C7(a)–C7(f)** pin the adaptation, including C7(e), which asserts against the real agent output schema that both defects are present before adaptation and neither after. Five probes redden them: the option set back to `true`, the option dropped, `propertyNames` kept, the root type not set, and the adaptation applied to Anthropic too. **One line is not covered offline** — the call in `generateStep` that routes the schema through the adapter. `Output.object` hides the schema in a closure, so no unit test can observe what was actually sent; the live run is its only proof.
