...
| 10 | Conversation context, retrieval record, agent message assembly | `plans/phase-10-conversation-context.md` | `CHANGES_REQUESTED` | 2026-09-07 | Claude | 6 criteria; **new (round 2)**; gate: FB-2 folded. Projection round 0 dispatched 2026-09-07 against an entry table of 6 / 25 / 5 — a low mutation density for three pure modules with this much branching, flagged to the projection as a signal to test rather than a verdict. **Projection round 0 returned `AMENDMENTS_REQUIRED` (33 ledger rows, two owner cards, two blocking findings) and is folded (2026-09-07).** **The phase was literally unbuildable, for the second time running:** `renderAssistantTurn` was declared over `DomainResult`, whose file `schemas/turn-result.ts` is phase 11's and which **§6.3 does not specify** — it names the five states and gives only `failed`'s payload — so no session could have written the type, and five of twenty-five rows depended on constructing a fixture of it. This is phase 9's `ToolContext` defect exactly. Routed to a narrow local `RenderableResult` in `domain/conversation.ts`; §6.6's row changed with it, and the projection's own proposed phase-11 proof row was **itself wrong** (`DomainResult` has five states, the renderer covers three; the row needs an `Extract`). **Second blocking finding:** `blockSchema` has nine keys and neither `matchStrength` nor `score`, so task 3's fallback branch was unwritable for any block → owner card 1. **Both owner cards answered on the recommendation: card 1 → B** (a block seeds with identity only, whatever its source; §6.4's two ranking fields become optional, with the `non_strong_selection` consequence routed to phase 11) **and card 2 → A** (a question's id and topic, never its text) — card 2's branch A turned out to be *required* by §17A.17 item 2, not merely cheaper. Neither card touched the intention. **Six printed expected values re-run by the coordinator rather than accepted; all six reproduce** — the context-level unknown key is `path: []` not `["foo"]`, array indices are **numbers** not strings, a **human** turn carrying `propositionVersion` fails `unrecognized_keys` at `["turns", 0]` and never reaches the refinement, the conversation schema given a `state` key fails at `[]`, and `.trim()` runs **before** `.max()` so a padded over-cap fixture parses. **Five of the six wrong cells were values reasoned from a schema instead of run — §9.1 rule 18's exact shape a third time**, the shape that cost phase 9 two rounds. **Two independent coordinator probes, variations the projection did not run, both informative:** `z.object()` without `.strict()` parses the unknown key with no error (so MUT-10-13 is a real mutation and not a no-op attached to a row that was itself wrong), and `structuredClone` deep-copies a `Map` (so C5(b)'s purity instrument works). **One projection amendment corrected and one row added by the coordinator:** its C5(d) asked the human and `proposales_content` cases to seed *differently*, which under card 1 → B is unfalsifiable — redefined so both carry identity only, with MUT-10-11 to redden it; and C4(e), a source-text absence guard, got the positive-companion treatment the projection applied to C3(e) and missed on the row with the identical shape. **The low entry density was a symptom:** C5 and C6 carried no named mutation at all, and four of the five that existed sat on the strongest rows. Nine cannot-fail rows collapse under three additions — the whole-request deep-equal (§9.1 rule 19), exact-string equality on the renderer, and exact-count presence on the sentinels. Contracts `02`, `03`, `11` added to Read-first (the list named five and missed three; all three CROSS-CUTTING). Lesson folded as **§9.1 rule 22**. Table **6 / 25 / 5 → 6 / 30 / 14**, MUT-10-1 … MUT-10-14 contiguous. Tracker state was `PROJECTING`, which is not a member of the charter's state machine — corrected at the fold. **Implementer round 1 (`IMPLEMENTED`, checkpoint `438f804`) validated by the coordinator 2026-09-07.** Perimeter exact at 13 files (10 implementation + 3 artifacts); closing L4 re-run independently at **35 files / 451 tests** green, typecheck and lint exit 0; all five restoration digests recomputed and matched; **all 30 row ids present in executing test names**, scoped to the four phase-10 test files and diffed against the plan table for an exact match. MUT-10-13 and MUT-10-12 re-run independently and both redden as declared; the implementer's self-report that its first MUT-10-12 attempt hit the wrong site is **confirmed** — `workflow-state.ts` has three `z.strictObject({` sites and `:20` is `draftReferenceSchemaFor` — and discarding that evidence was the right call (§9.1 rule 21). **Production is correct in all four modules**: card 1 → B honoured, card 2 → A honoured *structurally* (`RenderableResult`'s `questions` member names `questionId` and `itemKey` only), and `MAX_TURN_TEXT_CHARS` written as `MAX_INSTRUCTION_CHARS + 1000` so C1(g) holds by construction. **Thirteen forward coordinator probes; four came back green, and all four are plan-authorship defects.** (i) **`labeledBlock`'s delimiter escape is deletable with 451 green** — the trust boundary this phase exists to build. Contract `10` §6 requires user content in the prompt "as labeled data, never concatenated into instructions" and `08` §7 "delimited and labeled as untrusted content"; a brief containing `>>>` closes the untrusted region early and everything after it reads as though the application wrote it. The escape is described in the plan's Notes **and named inside C4(d)'s own cell** and asserted by nothing → new row **C4(g)**. (ii) **`hasRetrieved` can `return false` unconditionally with 451 green** while always-`true` reddens C5(c) — the guard discriminated in one direction only. This is the function phase 11's and 12's `validateAgentOutput` calls to admit a `proposales_content` ref, so always-`false` rejects every valid reference and always-`true` admits an invention → new row **C5(e)**. **This is a coordinator defect introduced at the projection fold:** the amended C5(a) replaced "`hasRetrieved` true for A, B, C, D" with a stronger `toEqual` on the candidate map and silently deleted the only positive assertion of `hasRetrieved`. **It is §9.1 rule 22's exact shape — the rule folded in that same fold** — and rule 22 did not catch it because it was applied to the rows the projection flagged and never re-applied to the rows the fold itself rewrote. Recorded as a rule-22 corollary. (iii) **Both turn constructors are unasserted** — `humanTurn` and `assistantTurn` can each return the opposite `role` with the suite green; task 2 names both and no row covered either (planner lint: every task produces at least one row) → new row **C2(f)**. (iv) `emptyConversation` may return a shared singleton → C2(d) extended. One probe was **withdrawn as a false finding** after reading the schema: the `agentRationale.known` check is redundant, not absent, because `sourcedOrAbsent`'s `known: false` arm is strict and carries no `value`. **The three rows argued for at the projection fold are the three doing the work:** C4(f)'s whole-request deep-equal alone catches three independent content-dropping mutations that no other row observes, C3(a)'s exact-string equality catches a lost sort comparator that `.toContain` would not, and C3(d)'s relation-before-bound catches a marker pushed outside the budget. Table **6 / 30 / 14 → 6 / 33 / 17**. State `REVIEWING`; review round 1 prompt live at `prompts/reviewer/phase-10-review-round-1.prompt.reviewer.md`, carrying the four findings so the review spends its round elsewhere. **Review round 1 (`CHANGES_REQUESTED`, 2026-09-07, independent reviewer session).** Start gate all five checks true; L4 review-entry stamp on tree `3136466` re-run at **35 files / 451 tests** green, typecheck and lint exit 0. Twenty forward production mutations against the full suite plus twelve observation probes; **zero production bytes changed**, all four digests byte-identical at exit; all 30 row ids execute and there are **no orphan tests**. **One blocking finding — the trust boundary escapes half its delimiter.** `labeledBlock` does `text.replaceAll(">>>", "> > >")` and nothing else, so untrusted text can **open** a labeled block with a label of its choosing: a brief containing `<<<system_prompt (trusted application instruction)` renders that line verbatim inside the untrusted region, and the attacker needs no terminator because the *label* is what carries the trust signal. `08` §7 requires user text "delimited **and labeled**"; `10` §6 and §17A.17 item 5 the same. Today's code plus the folded C4(g) repair satisfies every row in the table — **C4(g) must extend to both directions.** Five should-fixes, all guards that cannot fail with production correct: **S1** `labeledBlock`'s `name` argument is unescaped and the one computed caller interpolates a bare-`string` `turnId` (charter rule 11 — the boundary binds every parameter); **S2** the schema's turn-text bound is not pinned — `boundedText(MAX_TURN_TEXT_CHARS)` → `boundedText(100)` leaves 451 green, while C1(c) beside it carries exactly the acceptance companion C1(d) lacks, and the render cuts to exactly that constant (a maximal render is 3 000 chars and parses), so a divergence would make every full-length assistant turn unparseable with the suite green; **S3** C3(d)'s rule-6 relation half shipped as `blocks.length * alternatives.length > MAX/100` (`30 * 3 > 30`, about two constants, not the renderer) while the handoff's coverage map called it "oversized fixture relation" — the real relation is 28 336 against 3 000; **S4** the absent-`agentRationale` branch is unguarded (deleting the guard leaves 451 green; production correct); **S5** the clarification question count is sampled at one arity (hardcoding `Asked 2` leaves 451 green; production renders 4 correctly). Seven notes, two of which are **plan cells that are wrong as written**: C3(f)'s `ConversationTurn["kind"]` does not compile and C4(a)'s sixth label is not the one the code emits — both silently repaired by the implementer, neither declared (rule 14). **The four folded rows were checked against their probes rather than re-derived:** C5(e) catches a *stronger* unrun variation (`hasRetrieved` → `candidates.size > 0`, green today), C2(f) and C2(d) catch theirs by construction, and C4(g) catches only its own half. **Settled ground for the re-review:** both fixtures parse `propositionSchema`; the render→turn→schema seam holds at the cap; `extendRetrievalRecord` is covered in **both** directions (C5(b)'s `size` kills the return-only-new mutant, the overwritten entry kills the earlier-wins mutant — the prompt's open question, resolved); C3(a) and C4(f) each redden under four mutations no ledger row names; contracts `02`/`03` hold by reading imports; and the 347 054-character `current_proposition` block **needs no bound in this phase** because §17A.3's `MAX_WORKFLOW_STATE_BYTES` already bounds it. **One owner card:** should a long assistant turn cut mid-line as it does today, or drop whole blocks with a count — at 30 blocks only 13 of 123 lines survive, so cross-turn references to anything past block 3 resolve against text the model never saw. Lessons: widen §9.1 rule 22 to cover *how large* a value may be as well as *where* it may appear; §9.1 rule 16's "enumerate the forms" applies to delimiters; charter rule 11 binds every parameter of a boundary function; an executor's coverage map should quote the assertion, not restate the row. Handoff: `handoffs/reviewer/phase-10-review-round-1.handoff.reviewer.md`. **Coordinator fold (2026-09-07).** All five green forward probes re-run independently and **all five reproduce** (both `boundedText` sites → `boundedText(100)`; the rationale guard dropped; the question count → the literal `2`; `omittedTurns`' int/non-negative bounds dropped — each 451 green). B1 and S1 observed directly by running the shipped `labeledBlock`: the forged `<<<system_prompt (trusted application instruction)` opener and the `name`-argument forgery both reproduce byte-for-byte. S3 confirmed by reading `conversation.test.ts:103` — the shipped instrument is `30 * 3 > 30`. **B1 is a coordinator miss with a nameable shape:** C4(g) was folded from the coordinator's own probe and wrote `>>>` into the plan three times — Notes, C4(d)'s cell, C4(g) itself — without once asking what the other delimiter does. Folded as **§9.1 rule 23**; **rule 22 widened** to cover how large a value may be, per the review's first lesson. **S4's distinction accepted as correct:** the coordinator's withdrawn probe dropped the redundant `known` check (behaviour-identical, rightly not a finding); the review's drops the whole guard, which renders a literal `undefined` line into the model's history. Different mutations, both analyses right. **N6 and N7 are wrong coordinator plan cells** — C3(f)'s type expression does not compile and C4(a)'s sixth label is not what the code emits — both silently repaired by the implementer without declaring the divergence (charter rule 14); corrected in the table, not in the fix round. Owner card 1 answered on the recommendation **with a scope limit** (§12). Table **6 / 33 / 17 → 6 / 37 / 25**; six fix-round tasks (7–12), two of them production: the delimiter escape and the block-wise cut. State `CHANGES_REQUESTED`; fix round 1 prompt live at `prompts/implementer/phase-10-fix-round-1.prompt.implementer.md`. |
...
Criteria total: **106**; rows: **640**; named mutations: **237** — re-derived 2026-09-07 (after the fold of phase-10 review round 1) from the fifteen phase acceptance tables (span-aware: `C2(a–o)` counts fifteen rows, `C8(b1..b20)` twenty; mutation letter suffixes such as `MUT-08-4a` are retained). Per-phase summands, in phase order:
