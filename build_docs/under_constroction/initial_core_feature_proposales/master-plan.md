...
- The live suites are opt-in and were not run in the implementing session: the smoke writes a real draft, which is the owner's call. `PROPOSALES_EDITOR_ORIGIN` is therefore still unverified against the observed origin, and the smoke prints the observed value for exactly that reason.
- The configured provider is OpenAI. Whether its strict structured-output mode accepts the generated agent-output JSON schema is proven only offline by row Z1; the live eval is the true check.
