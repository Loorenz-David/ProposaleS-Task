...
| 9 | Agent runtime: tool definition, run loop, budgets, read tools | `plans/phase-09-agent-runtime.md` | `PROMPT_READY` | 2026-09-07 | coordinator | Rank-12 projection round 0 (`AMENDMENTS_REQUIRED`, 26 rows: P 16 · M 7 · I 1 · F 2) consumed and fully routed; owner card answered **branch 1**, closing §12's staged card with no intention or §6.3 change and without reopening approved phase 8. **The projection's central finding: a `run` that discarded every tool result passed all 22 original rows** — the loop's most important behaviour had no guard at all. Also unbuildable as written: nothing supplied `ToolContext`, so the loop could not invoke a tool (D02), and `get_content` had no representable output (D07). Table **6 / 22 / 4 → 7 / 34 / 13**; project **106 / 624 / 191**. Master §6.1, §6.3 (new closed `ToolErrorCode`), §6.4 (`ToolInvokeResult`, `RecordedToolCall`, `contentDetailSchema`, `requires`) and §6.6 (`run`'s `toolContext`, `defineTool`, `assertReadOnlyToolSet`, `PREPARATION_TOOLS`, the fake's `stepOptions`) amended. Perimeter 9 → **13 paths**, three of them additive changes to existing files. Coordinator re-ran the load-bearing claims: `z.toJSONSchema` does **not** throw on either real tool schema but silently drops `.refine()`, which narrowed D11's delegation and gave C5(b) a real production cause. §7.2 re-traced — C5 moves from M6 to M15 (D26), nothing left unserved. Prompt at `prompts/implementer/phase-09-round-1.implementer.md`. Zero tests, installs or provider calls in the projection. |
...
| `assertReadOnlyToolSet` | `features/proposal-preparation/server/tools/index.ts` | `(tools) → void`; throws `Error("tool set must contain only read tools: <names>")` on any non-`read` kind |
