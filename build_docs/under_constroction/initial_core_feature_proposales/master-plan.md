...
Criteria total: **106**; rows: **624**; named mutations: **191** — re-derived 2026-09-07 (after the phase-9 projection fold) from the fifteen phase acceptance tables (span-aware: `C2(a–o)` counts fifteen rows, `C8(b1..b20)` twenty; mutation letter suffixes such as `MUT-08-4a` are retained). Per-phase summands, in phase order:

- criteria `5 + 7 + 6 + 8 + 8 + 8 + 8 + 7 + 7 + 6 + 8 + 8 + 7 + 8 + 5`
- rows `22 + 52 + 51 + 80 + 61 + 54 + 57 + 51 + 34 + 25 + 28 + 33 + 26 + 32 + 18`
- mutations `11 + 19 + 16 + 35 + 21 + 5 + 21 + 22 + 13 + 5 + 7 + 4 + 4 + 4 + 4`
...
| `ToolErrorCode` | `invalid_arguments` (the tool's **input** failed its schema; carries `issues`), `invalid_tool_output` (the tool's `execute` returned a value failing its own output schema; 08 §3), `language_unresolved` (a `requires(ctx)` precondition refused the call because `ctx.language` is null, so the model must derive or ask first) — 3 members, closed. These were used by phase 9's plan and registered nowhere; phases 11 and 12 read this union | `src/lib/agent/types.ts` / phase 9 |
...
| `ToolInvokeResult<O>` | `{ ok: true, value: O } \| { ok: false, error: { code: "invalid_arguments", issues } } \| { ok: false, error: { code: "invalid_tool_output" } } \| { ok: false, error: { code: ToolErrorCode } }` — three failure arms, the third produced only by `requires`. **`invoke` does not catch a throw from `execute`**: a thrown value is a programming error and propagates (contract `04` §6, D05) |
