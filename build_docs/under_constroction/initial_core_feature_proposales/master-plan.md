...
| 8 | AI provider boundary (`@/lib/ai`) | `plans/phase-08-ai-provider-boundary.md` | `REVIEWING` | 2026-09-06 | coordinator | Round 1 implemented by Codex at checkpoint `6441770` and consumed. 6 criteria / 47 rows / 16 distinct mutations; all 47 row ids appear in executing test names. Coordinator re-ran the stamp on the checkpoint tree: **28 files / 380 tests** green (+4 files / +45 tests over the 24 / 335 baseline), typecheck and lint clean; perimeter exact by diff. Three independent coordinator probes: two bit as intended (branch reorder → C4(j)/C6(j); other-4xx → `transport` → C4(d)/C4(e)), and **P3 found a guard that proves a copy of itself** — weakening C2(b)'s own forbidden-form regex left the file 9/9 green including C2(c), the instrument-proof row. Routed to the reviewer as a named probe, with one open semantic question on C6(k)'s `invalid_response`. Vendor versions folded into §10.1; `ai` stayed 7.0.92. |
