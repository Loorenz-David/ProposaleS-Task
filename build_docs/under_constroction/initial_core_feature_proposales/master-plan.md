...
18. **Where a row names an upstream failure, the fixture is the object the upstream library actually builds — located in its source — never a plausible stand-in.** This is a third species of unfalsifiable guard, and the worst so far: not two sufficient causes for the observed outcome, but **zero production causes** for it. The row passes, the mutation reddens it, the mutation log looks complete, and the path it claims to cover has never once been executed. Phase 8 wrote "a `TypeError` raised by the SDK invocation" and "an error with no status carrying a decode cause" from the intention's prose; the vendor SDK builds neither. A real network failure arrives as a **status-less `APICallError`** and a real undecodable reply as an `APICallError` carrying the **2xx** status — so the network row mapped to a reasonless nonretryable error and the reason the owner had just ratified for undecodable replies was **unreachable**, with every test green. Open the library, find the construction site, cite it in the fixture cell. This applies to every remaining boundary row in the project: phase 9's tool failures, phases 11–14's Proposales errors. (Earned 2026-09-06, phase 8 review B2/B3, proven end-to-end through the real provider with only `fetch` injected.)
