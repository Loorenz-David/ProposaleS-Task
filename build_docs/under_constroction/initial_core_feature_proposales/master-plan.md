...
| 8 | AI provider boundary (`@/lib/ai`) | `plans/phase-08-ai-provider-boundary.md` | `IMPLEMENTED` | 2026-09-06 | coordinator | Review round 1 consumed and folded; fix round 2 implemented by Codex. **B2 and B3 are the round's real finding and I re-derived both on an independent harness** — driving the real `@ai-sdk/anthropic` provider and the real `generateText` with only `fetch` injected, a DNS/connect failure produced an `IntegrationError` with **no reason** and `retryable: false` where §17A.13 requires `transport`/`true`, and an undecodable **200** produced `request_rejected` with `status: 200`, so `invalid_response` — the member the owner ratified at round 17 for exactly that condition — was **unreachable on the real path**, with every test green. Cause: `C4(h)`/`C4(i)` were written from the intention's prose and tested shapes the vendor SDK never builds (confirmed at `@ai-sdk/provider-utils/dist/index.js:472` and `:3999`). The reviewer's prescription was applied and measured by the coordinator (E1 → `transport`, E2 → `invalid_response`, control unchanged, suite green, typecheck clean) and reverted. B1 = coordinator P3, repaired onto one shared instrument; S1 = deleting the client's content-filter branch left the suite green. Table **6 / 47 / 16 → 7 / 51 / 22**; project **105 / 612 / 182**. Intention §23 round 18 adds the AI table's **precedence** paragraph (editorial, gate closed); master §9.1 gains **rules 17 and 18** plus two planner lints. Owner card 1 answered and staged to phase 9 (§12). Closing L4: 28 files / 383 tests green; typecheck and lint clean. Under §9.0.2 no re-review follows the fix. |
