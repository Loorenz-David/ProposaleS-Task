...
| 9 | Agent runtime: tool definition, run loop, budgets, read tools | `plans/phase-09-agent-runtime.md` | `NOT_STARTED` | 2026-09-06 | coordinator | 6 criteria / 22 rows / 4 mutations, re-derived by command. **Mandatory projection (rank 12, the project's highest) dispatched** at `prompts/reviewer/phase-09-projection-round-0.prompt.reviewer.md`; implementation gate closed until its ledger is folded. Two inherited obligations carried into it: the `search_content` tool's bound must **be** `MAX_SEARCH_QUERY_CHARS` rather than a second literal (phase-7 owner card), and the tool boundary's purity guard inherits the four-form list plus §9.1 rule 17's shared-symbol requirement. The phase-8 owner card (truncated-generation semantics) is staged here and the projection re-puts it with branches restated; the owner's recorded leaning in §12 is not the decision. Coverage adequacy is an explicit ask: 22 rows guard the bounded loop, three budgets, usage accumulation, one retry and two tools, where phase 8's error boundary needed 51. |
...
Criteria total: **105**; rows: **612**; named mutations: **182** — re-derived 2026-09-06 (after the phase-8 review fold) from the fifteen phase acceptance tables (span-aware: `C2(a–o)` counts fifteen rows, `C8(b1..b20)` twenty; mutation letter suffixes such as `MUT-08-4a` are retained). Per-phase summands, in phase order:

- criteria `5 + 7 + 6 + 8 + 8 + 8 + 8 + 7 + 6 + 6 + 8 + 8 + 7 + 8 + 5`
- rows `22 + 52 + 51 + 80 + 61 + 54 + 57 + 51 + 22 + 25 + 28 + 33 + 26 + 32 + 18`
- mutations `11 + 19 + 16 + 35 + 21 + 5 + 21 + 22 + 4 + 5 + 7 + 4 + 4 + 4 + 4`
...
| M15 | 8.C5, 8.C7, 9.C3, 9.C4, 11.C2, 11.C6, 12.C7, 12.C8 |
...
| Card 1 of the phase-8 review — **answered, staged (2026-09-06)** | Where a **truncated generation** belongs. A step that finishes with `length` and no usable output is reported today as a provider failure (`invalid_response`), which reads as "the provider's reply could not be decoded" when in fact the reply decoded perfectly and was merely cut off. The owner confirmed the recommendation: **phase 8 keeps today's behavior** (nonretryable, safe) and the semantic question goes to **phase 9's projection**, where the run loop and `MAX_OUTPUT_RETRIES` consume it. **Unresolved and deliberately not guessed:** the review's recommendation was branch 3 (*not a provider failure at all — an unusable answer the run loop owns*), and the owner's message added *"i already see branch 2 being the correct one for phase 9"*, which as written is branch 2 (*add a tenth `AiProviderFailureReason` member*). Those two point opposite ways on whether the round-17 registry reopens. Phase 9's projection **re-puts this card with the branches restated**, and the owner's leaning is recorded here verbatim rather than interpreted. | Phase 8: none — it closes on today's behavior. Phase 9: projection re-puts the card before writing its output-retry rows; the answer may amend §17A.13 and master §6.3. |
