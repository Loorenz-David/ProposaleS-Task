...
| 7 | Content ranking domain and human search | `plans/phase-07-ranking-and-human-search.md` | `PROMPT_READY` | 2026-09-06 | coordinator | Projection round 0 `AMENDMENTS_REQUIRED` (33 ledger rows) folded 2026-09-06. Owner card 1 → add the cap: `MAX_SEARCH_QUERY_CHARS` (intention §17A.16, §23 round 15; header stays `RATIFIED`). Registry amendments made here before dispatch: §6.4 `searchContentInputSchema`, §6.5 `MAX_SEARCH_QUERY_CHARS`, §6.6 `scoreItem` split out of `strength.ts`, `tokenize` / `catalogLanguages` registered, `searchContentForHuman` takes `unknown` with a getter-based `defaultDeps`. Phase grew 7 → 8 criteria, 28 → 56 rows, 3 → 14 mutations; perimeter 7 → 9 paths. Phase-5 N6 is discharged by the new C8, not by C7(d) |
