...
| 7 | Content ranking domain and human search | `plans/phase-07-ranking-and-human-search.md` | `IMPLEMENTED` | 2026-09-06 | Claude | Independent first review of checkpoint `f2399ac` at tree `e621226`: all 6 gate items pass; counts re-derived 8/56/14; 56 tests trace bijectively to the 56 rows, no orphan, no uncovered row; fixture roster and the plan's worked score table verified reproduced. **Three blocking rule-15 findings**, each proven by a planted mutant that left 171/171 green: B1 the comparator reduced to `variationId asc` (C6(a)-(c) satisfied by fixture coincidence); B2 no row observes `score` or `matchStrength`, so the scoring arithmetic is unpinned; B3 the missing-key half of the language filter is unguarded and C5(a) cannot fail (the shape the coordinator left to this review). Should-fix: S1 C7(d)'s `expectTypeOf` never fires; S2 C1(c) is blind to dynamic `import()`. Notes N1-N4. Verified corrections exist for all five on the shipped fixture. No L4 taken: `src/` byte-identical to the implementer's stamp, cited not re-run; 12 probes applied and reverted, checksums confirmed. Review round 1 `CHANGES_REQUESTED` (3 blocking, 2 should-fix, 4 notes) folded 2026-09-06: C1(c), C5(a), C6(a–c), C7(d) rewritten, C6(f) added; 56 → 57 rows, 14 → 19 mutations. Two of the review's stated corrections were verified wrong or incomplete before being quoted into the fix prompt (B1's query cannot carry C6(a)/C6(c); MUT-07-17 read literally throws) — findings stand, prescriptions repaired. Fix round 2 dispatched at `prompts/implementer/phase-07-fix-round-2.implementer.md`; **no independent re-review will follow it, by owner direction 2026-09-06** — the coordinator validates and closes. Fix round 2 `IMPLEMENTED` 2026-09-06 (Claude Fable 5.1): B1, B2, B3, S1, S2 resolved test-side only — two test files changed, zero production or fixture changes; C6(a–c) re-sited onto the discriminating queries as `(variationId, score, matchStrength)` tuples, C6(f) added, C5(a) control + `ledningsnivå`, C7(d) source-text guard, C1(c) dynamic-`import(` form. Mutations executed = declared = 19 (5 new at L1, MUT-07-2 and MUT-07-10 re-run, 12 cited from round 1 on untouched production code) plus 4 declared self-chosen probes; every probe reverted and verified. Closing L4: `npm test` 24 files / 335 tests green (+1 = C6(f); the phase's four files carry 12+12+22+11 = 57 = rows), typecheck and lint exit 0. **Two observed-red deltas declared for the coordinator's validation:** MUT-07-15 reddens C6(c), not C6(a) as the amended cell says (structural — a strong/possible pair is ordered by the strength term before the score term is consulted; the reviewer's both-terms probe reddens C6(a)); MUT-07-17's red is a `TypeError` thrown when the admitted item reaches projection, with a two-site probe confirming the `[]` assertion itself bites. No re-review follows (owner direction). |
...
Criteria total: **104**; rows: **587**; named mutations: **162** — re-derived 2026-09-06 (after the phase-7 review round-1 fold) from the fifteen phase acceptance tables (span-aware: `C2(a–o)` counts fifteen rows, `C8(b1..b20)` twenty). Per-phase summands, in phase order:

- criteria `5 + 7 + 6 + 8 + 8 + 8 + 8 + 6 + 6 + 6 + 8 + 8 + 7 + 8 + 5`
- rows `22 + 52 + 51 + 80 + 61 + 54 + 57 + 26 + 22 + 25 + 28 + 33 + 26 + 32 + 18`
- mutations `11 + 19 + 16 + 35 + 21 + 5 + 19 + 4 + 4 + 5 + 7 + 4 + 4 + 4 + 4`
...
15. **An exclusion row's fixture must make the predicate under test the only reason for exclusion — and in this project the score floor is the standing second cause.** Every "item X is not a candidate" row has two paths to the same observable: the filter it means to test, and `T_FLOOR`. A fixture whose query the excluded item cannot answer *at all* satisfies both, so deleting the filter leaves the row green. Construct the query from text the excluded item **does** carry in the language under test, so that removing the filter admits it above the floor. (Earned 2026-09-06, phase 7 C5(b): the mutation run itself caught the false green, which is the only reason it was not shipped. Phase 7's review then found the identical defect in C5(a), which no mutation had forced anyone to check. The same shape is live wherever a language, policy, or eligibility filter is asserted by absence — phases 9, 11 and 12 all have such rows.)
    ...
    **Corollary for the planner:** a sort key that is a monotone function of a later key in the same tuple can never be discriminated by an ordering row. Either drop it, or state which row guards it by asserting the derived label directly.
