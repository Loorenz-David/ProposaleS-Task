...
**Precedence, so the table is decidable and not merely total** (§23 round 18). Apply the same principle the Proposales table above already carries: **a non-2xx reply is classified by its HTTP status, whatever its body**, and a reply that never reached the provider carries no status at all. Therefore:

1. A reply that arrived with a **2xx** status but cannot be decoded as the provider's own protocol is `invalid_response` — the status is not evidence of success once the body is unreadable.
2. A reply that arrived with a **non-2xx** status is classified by that status even when its body is also undecodable. A `403` whose body is malformed is still `request_rejected`; the decode failure never upgrades or downgrades a status classification.
3. A failure carrying **no status** never reached the provider and is `transport` — unless it is a local abort or timeout, which is checked first because an aborted request also has no status.

This ordering is stated because the vendor SDKs express all three as the same error class distinguished only by the presence and value of a status field, so a classifier that reads the class alone cannot tell them apart. Verified against `@ai-sdk/provider-utils`: a fetch-level failure becomes a status-less error, and an undecodable successful reply becomes an error carrying the original **2xx** status with the decode failure as its cause.
...
**"Explicitly", as code shape.** `src/lib/ai/` resolves a **provider instance** from configuration and passes a **model instance** to every SDK call. A string model id is unrepresentable at the call site: the internal `generate` / `generateStructured` / `stream` signatures accept the SDK's language-model type **with the string id excluded** — `Exclude<LanguageModel, string>`, named `LanguageModelInstance`. **Corrected 2026-09-06 (follow-up 4, editorial):** this sentence previously read "the SDK's language-model type, which a `string` does not satisfy", which is inaccurate against `ai@7.0.92` — its `LanguageModel` alias *includes* `GlobalProviderModelId`, so accepting the bare alias would accept a string. The mechanism is unchanged and the exclusion is what enforces it. One registry, keyed by the configured provider enum, is the only place a model id becomes a model. `globalThis.AI_SDK_DEFAULT_PROVIDER` is **never assigned** — assigning it would install a second, hidden selection mechanism.
...
- **Every free-text field has a named maximum length and is trimmed** at the schema: brief, revision instruction, title, narrative, per-block reviewer comment, alternative reason, commercial-note text, question text, answer text, agent rationale, warning text, assumption note, `ref.quote`, and the **content-search query**. The brief cap and the alternatives cap are set so a conforming workflow cannot reach `MAX_WORKFLOW_STATE_BYTES` (§17A.3) by ordinary use.
- **The content-search query is one bound, shared by both search paths.** The text a human types into content search and the `query` the model passes to the `search_content` tool are the same free-text field entering the same ranking function, so they carry the **same named maximum**. Unbounded on the human path, an overlong paste is not refused but silently diluted — a query of two thousand words drives every real match below `T_floor` (§17A.8), so the human sees an arbitrary-looking list with nothing logged as wrong, while the assistant's identical paste is refused. Two paths disagreeing about what a query *is* is the defect this bound prevents; the numeric value is an implementation-plan choice (owner decision, phase-7 projection card 1, 2026-09-06).
...
**Round 15 (2026-09-06, phase-7 projection owner decision).** Status stays `RATIFIED`.

- **Card 1 → add the cap.** The owner selected the recommended option: the content-search query carries a named maximum length. This applies an **already-ratified standing rule** — §17A.16's first bullet requires every free-text field to be named, bounded, and trimmed at the schema — to the one field the enumeration missed, so it is an omission repair rather than a new mechanism. §17A.16's first bullet now names the content-search query, and a new paragraph states that the human search box and the model's `search_content` tool share the one bound.
- **The numeric value is not ratified here**, following the round-13 precedent for the alternative reason: the intention owns *that* the field is bounded; the implementation plan owns the number (`MAX_SEARCH_QUERY_CHARS`, master plan §6.5).
- No capability, endpoint, persistence, transport, or external call changed, so the ratification gate remains closed.
...
**Round 18 (2026-09-06, phase-8 review round 1, editorial).** Status stays `RATIFIED`.

- **§17A.13's AI table gains a precedence paragraph.** Round 17 made the map total — every condition has a reason — but not *decidable*: it did not say which reason wins when a reply is both non-2xx and undecodable, nor how a failure carrying no status at all is separated from one that does. The review proved this was not theoretical. Driven end-to-end through the real vendor provider with only `fetch` injected, a DNS/connect failure produced no reason at all and reported itself nonretryable, and an undecodable **200** was labelled `request_rejected` — a member master §6.3 defines as covering non-`429` **4xx**. The reason the owner added at round 17 for exactly that condition, `invalid_response`, was unreachable on the real path.
- The paragraph applies the **already-ratified** transport-precedence principle that §17A.13's Proposales table above has carried since round 12, to the AI table that was written without it. No reason is added, removed or redefined and no capability changes, so this is editorial and the ratification gate stays closed. Phase 8's fix round implements it and writes one row per branch against fixtures taken from the SDK's own error-construction source rather than from this prose.
