...
3. `tools/search-content.tool.ts`: kind `read`; input `{ query: z.string().trim().min(1).max(200) }`; output `{ candidates: z.array(contentCandidateSchema) }`; `execute` = `rankCandidates(input.query, ctx.catalog, ctx.language ?? throw)` — when `ctx.language` is null the tool returns an `invalid_arguments`-class error `language_unresolved` so the model asks or derives first. `tools/get-content.tool.ts`: input `{ variationId: z.string().regex(/^\d+$/) }`; output `{ item: candidate-shaped item | null }` from `ctx.catalog`.
