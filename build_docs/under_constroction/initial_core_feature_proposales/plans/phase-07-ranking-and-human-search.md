...
| C6(a) | strong before possible | `rankCandidates("consulting service track", FIXTURE_CATALOG, "en")` | `["2", 1000, "strong"]` precedes `["1", 667, "possible"]` — asserted as `(variationId, score, matchStrength)` tuples. **The strong item carries the higher id**, so ascending `variationId` cannot produce this order | **MUT-07-20** `rank-candidates.ts` · comparator, definition · delete **both** the strength and the score term, reducing it to `variationId` ascending → C6(a) red (`expected 1 to be less than 0`), and C6(b) red independently (`expected 4 to be less than 2`). **Corrected by fix round 2:** the cell named MUT-07-15 (score term only), which cannot redden this row — C6(a)'s pair is `strong` versus `possible`, so the strength term orders it before the score term is ever consulted. That is the mirror of the redundancy argument in Notes, and it is the coordinator's transcription error: review round 1 assigned MUT-07-15 to C6(c), which is where it now sits | M12 |
| C6(b) | possible before weak | `rankCandidates("consulting service bundle", FIXTURE_CATALOG, "en")` | `["5", 444, "possible"]` precedes `["3", 333, "weak"]` — tuples, and id 5 > id 3 | **MUT-07-20** (above) → C6(b) red on its own assertion | M12 |
| C6(c) | equal strength, higher score first | the same `"consulting service bundle"` ranking; both items are `possible` | `["8", 667, "possible"]` precedes `["5", 444, "possible"]` — tuples, and id 8 > id 5 | **MUT-07-15** `rank-candidates.ts` · comparator, definition · delete the score term (`if (a.score !== b.score) return b.score - a.score;`) → C6(c) red (`expected 3 to be less than 2`). A same-band pair is the **only** pair that can observe the score term | M12 |
...
Criteria: 8 (C1–C8), 57 rows (a table line is one row; a lettered span counts its letters). Named mutations: **21 distinct** (MUT-07-1 … MUT-07-21, no gaps). Per-criterion mutation *occurrences*: C1 5 · C2 3 · C3 1 · C4 1 · C5 4 · C6 6 · C7 2 · C8 1 = **23**, which exceeds 21 because **MUT-07-20 is cited by both C6(a) and C6(b)** — permitted by master §9.1 rule 14, because each row's failing assertion was independently observed (`expected 1 to be less than 0` and `expected 4 to be less than 2`). Re-derived by command after the fix round-2 fold; counted, not carried.
...
**Coordinator validation and approval (2026-09-06).** Tracker row 7 → `APPROVED` at `5949615`.
**Caveat, recorded because it is load-bearing: no independent re-review followed fix round 2**
(owner direction, 2026-09-06). This entry is the gate.

- **Perimeter verified against the tree.** `0d474cb` touches four files (two test files, two
  documents); `5949615` touches one line of this Review log. `git diff f2399ac HEAD` over the five
  production files — `rank-candidates.ts`, `strength.ts`, `content-candidate.ts`, `catalog.ts`,
  `search-content-for-human.ts` — is **empty**: the round was test-side, as its perimeter required.
  `git status --porcelain` carries only the fix handoff.
- **Independent probes, chosen as variation the fix round did not run.** (1) The **denominator**
  mutation `3 * queryTokens.size` → `4 *` — the second shape B2 named, where the fix round ran only
  the weight change: C6(a), C6(b), C6(c) and C6(f) all red, 4 failed / 18 passed. (2) A **type-only**
  model import (`import type { AiClient } from "@/lib/ai"`) added to the service: C7(d) red, 1 failed
  / 10 passed — the row genuinely covers `import type`, which is what S1's correction claimed and
  what MUT-07-18 (a value import) did not by itself prove. Both files restored and verified
  byte-identical by SHA-256.
- **Approval-gate L4 stamp**, taken on the tree being approved (charter L4(c)): HEAD `5949615`,
  `git status --porcelain` clean apart from the untracked fix handoff. `npm test` → **24 files /
  335 tests green**; `npm run typecheck` exit 0; `npm run lint` exit 0. Failure-ID delta ∅ → ∅
  against the round-1 baseline of 334.
- **Three ledger corrections folded from the fix round, all of them real.** MUT-07-15 was on C6(a),
  which cannot observe it — a `strong`-vs-`possible` pair is settled by the strength term before the
  score term is consulted. **That was the coordinator's transcription error**: review round 1
  assigned it to C6(c), and it is back there; C6(a) and C6(b) now carry MUT-07-20, the reduced
  comparator, with independently observed reds. MUT-07-17's red is a `TypeError`, but one *caused
  by* the admitted item rather than by the crash shape the cell warns against, so MUT-07-21 (the
  two-site variant) is named as the row's proof that the assertion itself bites. MUT-07-19's literal
  `await import(...)` does not parse inside a synchronous function; the cell now names
  `void import(...)`.
- **Counts re-derived by command:** 8 criteria, 57 rows, **21 distinct** mutations with no gaps;
  per-criterion occurrences sum to 23 because MUT-07-20 serves two rows (§9.1 rule 14, both reds
  independently observed). Project totals: 104 / 587 / 164.
- **The fix round's own provenance discipline is worth recording:** it caught a typed summand line
  in its first commit and corrected it in a second commit rather than amending, so the record shows
  what happened. That is the right handling of charter manifest property 3.
- **Lesson folded upward:** master §9.1 rule 15's planner corollary gains its mirror — an ordering
  row's named mutation must be one that row's own pair can observe.
- **Open items leaving this phase:** N1 → phase 11 (`reason` computed over the untruncated
  description); N2 → phase 12 (numerically equal `variationId`s tie, accepted MVP limit with its
  reachability argument recorded); phase 9 inherits the complete source-guard form list. Phase-5 N6
  is **closed** — discharged by C8's twelve rows.
